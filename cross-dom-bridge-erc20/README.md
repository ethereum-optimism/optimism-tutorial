...
The SDK provides transparent safety rails to prevent some of those mistakes.
...
Once the message is relayed the balance change on OP (Mainnet or Goerli) is practically instantaneous.
We can just report the balances and see that the L2 balance rose by 1 gwei.

### `withdrawERC20`

This function shows how to withdraw ERC-20 from OP Mainnet to Ethereum (or from OP Goerli to Goerli).
The withdrawal process has these stages:

1. Submit the withdrawal transaction on OP (Mainnet or Goerli).
...
This is the initial withdrawal transaction on OP Goerli.
